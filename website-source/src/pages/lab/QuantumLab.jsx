...
export default function QuantumLab() {
  ...
  // Backend API options
  const [checkMode, setCheckMode] = useState('variants');
  const [searchMode, setSearchMode] = useState('random');
  const [maxPermutations, setMaxPermutations] = useState(1000000);
  const [maxValidPermutations, setMaxValidPermutations] = useState(1000);
    ...
    try {
      ...
      if (data.k === 0) {
        ...
        const backendPayload = {
          ...
          max_permutations: Number(maxPermutations),
          max_valid_permutations: Number(maxValidPermutations)
        };
    ...
    <>
      <div className="app-content">
        <header className="glass-panel">
          {/* Top nav row — always visible */}
          <div className="toolbar-nav-row">
            <div className="toolbar-nav-left">
              <button 
                onClick={() => navigate(-1)} 
                className="home-link" 
                style={{ background: 'transparent', border: 'none', padding: 0, margin: 0, boxShadow: 'none' }}
                aria-label="Go back"
              >
                &larr; Back
              </button>
              <RouterLink to="/" className="home-link" aria-label="Go to homepage" style={{ color: 'var(--text-muted)' }}>
                Home
              </RouterLink>
            </div>
            <div className="editor-title">Graph Code Editor</div>
            <div className="toolbar-nav-right">
              <button className="primary btn-xs" onClick={generateMatrices} title="Generate">
                <Play size={14} /> <span className="btn-label-full">Generate</span>
              </button>
              <button className="btn-xs" onClick={clearAll} title="Clear All">
                <RefreshCw size={14} /> <span className="btn-label-full">Clear</span>
              </button>
              <button className="btn-xs" onClick={handleSaveJson} title="Save JSON">
                <Save size={14} /> <span className="btn-label-full">Save</span>
              </button>
            </div>
          </div>

          {/* Mode button grid — 3 columns: Add | Edit | Del */}
          <div className="toolbar-mode-grid">
            <div className="mode-col">
              <span className="mode-group-label">Add</span>
              <button className={classNames('btn-xs mode-pill', mode === 'add_cluster' && 'active-mode')} onClick={() => handleModeChange('add_cluster')}>
                <PlusCircle size={13} /> Cluster
              </button>
              <button className={classNames('btn-xs mode-pill', mode === 'add_message' && 'active-mode')} onClick={() => handleModeChange('add_message')}>
                <PlusSquare size={13} /> Message
              </button>
            </div>

            <div className="mode-col">
              <span className="mode-group-label">Edit</span>
              <button className={classNames('btn-xs mode-pill', mode === 'move' && 'active-mode')} onClick={() => handleModeChange('move')}>
                <Move size={13} /> Move
              </button>
              <button className={classNames('btn-xs mode-pill', mode === 'connect_cc' && 'active-mode')} onClick={() => handleModeChange('connect_cc')}>
                <LinkIcon size={13} /> C–C
              </button>
              <button className={classNames('btn-xs mode-pill', mode === 'connect_mc' && 'active-mode')} onClick={() => handleModeChange('connect_mc')}>
                <LinkIcon size={13} /> M–C
              </button>
            </div>

            <div className="mode-col">
              <span className="mode-group-label">Del</span>
              <button className={classNames('btn-xs mode-pill', mode === 'delete_node' && 'active-mode')} onClick={() => handleModeChange('delete_node')}>
                <Trash2 size={13} /> Node
              </button>
              <button className={classNames('btn-xs mode-pill', mode === 'delete_link' && 'active-mode')} onClick={() => handleModeChange('delete_link')}>
                <Unlink size={13} /> Link
              </button>
            </div>
          </div>
        ...
        <div className="workspace">
          ...
          <div className="terminal-panel glass-panel">
            ...
            <div style={{ padding: '12px 14px 8px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontWeight: 600, marginBottom: 8 }}>Backend Analysis Options</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <label style={{ fontSize: 12, opacity: 0.85 }}>Check Mode</label>
                  <select value={checkMode} onChange={(e) => setCheckMode(e.target.value)} style={{ padding: '4px', borderRadius: '4px', background: '#f8fafc', color: '#0f172a' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <label style={{ fontSize: 12, opacity: 0.85 }}>Search Mode</label>
                  <select value={searchMode} onChange={(e) => setSearchMode(e.target.value)} style={{ padding: '4px', borderRadius: '4px', background: '#f8fafc', color: '#0f172a' }}>
                    <option value="random">Random</option>
                    <option value="lexicographic">Lexicographic</option>
                  </select>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <label style={{ fontSize: 12, opacity: 0.85 }}>Max Perms</label>
                  <input type="number" value={maxPermutations} onChange={(e) => setMaxPermutations(e.target.value)} style={{ padding: '4px', width: '80px', borderRadius: '4px', background: '#f8fafc', color: '#0f172a' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <label style={{ fontSize: 12, opacity: 0.85 }}>Max Valid Perms</label>
                  <input type="number" value={maxValidPermutations} onChange={(e) => setMaxValidPermutations(e.target.value)} style={{ padding: '4px', width: '80px', borderRadius: '4px', background: '#f8fafc', color: '#0f172a' }} />
                </div>
              </div>
            </div>
